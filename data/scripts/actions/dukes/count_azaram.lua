	...
	if item.itemid == 9825 then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.GraveDanger.CountAzaram) > os.time() then
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.GraveDanger.CountAzaram, os.time() + 20*60*60) -- 20 hours
		end
